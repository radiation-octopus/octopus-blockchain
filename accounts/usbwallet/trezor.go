...
import (
	...
	"github.com/radiation-octopus/octopus-blockchain/accounts/usbwallet/trezor"
...
func (w *trezorDriver) Derive(path accounts.DerivationPath) (entity.Address, error) {
	return w.trezorDerive(path)
}

// SignTx implements usbwallet.driver, sending the transaction to the Trezor and
// waiting for the user to confirm or deny the transaction.
func (w *trezorDriver) SignTx(path accounts.DerivationPath, tx *block.Transaction, chainID *big.Int) (entity.Address, *block.Transaction, error) {
	if w.device == nil {
...
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (entity.Address, error) {
	...
	if addr := address.GetAddressBin(); len(addr) > 0 { // Older firmwares use binary fomats
		return entity.BytesToAddress(addr), nil
	}
	if addr := address.GetAddressHex(); len(addr) > 0 { // Newer firmwares use hexadecimal fomats
		return entity.HexToAddress(addr), nil
	}
	return entity.Address{}, errors.New("missing derived address")
}

// trezorSign sends the transaction to the Trezor wallet, and waits for the user
// to confirm or deny the transaction.
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *block.Transaction, chainID *big.Int) (entity.Address, *block.Transaction, error) {
...
func (w *trezorDriver) trezorExchange(req proto.Message, results ...proto.Message) (int, error) {
	...
	return 0, fmt.Errorf("trezor: expected reply block %s, got %s", expected, trezor.Name(kind))
}
