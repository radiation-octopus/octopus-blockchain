...
type driver interface {
	...
	Derive(path accounts.DerivationPath) (entity.Address, error)

	// SignTx sends the transaction to the USB device and waits for the user to confirm
	// or deny the transaction.
	SignTx(path accounts.DerivationPath, tx *block.Transaction, chainID *big.Int) (entity.Address, *block.Transaction, error)

	SignTypedMessage(path accounts.DerivationPath, messageHash []byte, domainHash []byte) ([]byte, error)
}

// wallet represents the entity functionality shared by all USB hardware
...
type wallet struct {
	...
	paths    map[entity.Address]accounts.DerivationPath // Known derivation paths for signing operations

	deriveNextPaths []accounts.DerivationPath // Next derivation paths for account auto-discovery (multiple bases supported)
	deriveNextAddrs []entity.Address          // Next derived account addresses for auto-discovery (multiple bases supported)
...
func (w *wallet) Open(passphrase string) error {
	...
	w.paths = make(map[entity.Address]accounts.DerivationPath)
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		var (
			...
			nextAddrs = append([]entity.Address{}, w.deriveNextAddrs...)
...
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain ethereum.ChainStateReader) {
	...
	w.deriveNextAddrs = make([]entity.Address, len(bases))
	w.deriveChain = chain
}
...
func (w *wallet) SignTx(account accounts.Account, tx *block.Transaction, chainID *big.Int) (*block.Transaction, error) {
	w.stateLock.RLock() // Comms have own mutex, this is for the state fields
	defer w.stateLock.RUnlock()
...
// Since USB wallets don't rely on passphrases, these are silently ignored.
func (w *wallet) SignTxWithPassphrase(account accounts.Account, passphrase string, tx *block.Transaction, chainID *big.Int) (*block.Transaction, error) {
