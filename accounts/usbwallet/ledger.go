...
func (w *ledgerDriver) Derive(path accounts.DerivationPath) (entity.Address, error) {
...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *block.Transaction, chainID *big.Int) (entity.Address, *block.Transaction, error) {
	// If the Ethereum app doesn't run, abort
	if w.offline() {
		return entity.Address{}, nil, accounts.ErrWalletClosed
	}
	// Ensure the wallet is capable of signing the given transaction
	if chainID != nil && w.version[0] <= 1 && w.version[1] <= 0 && w.version[2] <= 2 {
		//lint:ignore ST1005 brand name displayed on the console
		return entity.Address{}, nil, fmt.Errorf("Ledger v%d.%d.%d doesn't support signing this transaction, please update to v1.0.3 at least", w.version[0], w.version[1], w.version[2])
	}
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (entity.Address, error) {
	...
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return entity.Address{}, errors.New("reply lacks public key entry")
	}
	reply = reply[1+int(reply[0]):]

	// Extract the Ethereum hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return entity.Address{}, errors.New("reply lacks address entry")
	}
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *block.Transaction, chainID *big.Int) (entity.Address, *block.Transaction, error) {
	// Flatten the derivation path into the Ledger request
	path := make([]byte, 1+4*len(derivationPath))
	path[0] = byte(len(derivationPath))
	for i, component := range derivationPath {
	...
	if len(reply) != crypto.SignatureLength {
		return entity.Address{}, nil, errors.New("reply lacks signature")
	}
	signature := append(reply[1:], reply[0])

	// Create the correct signer and signature transform based on the chain ID
	var signer block.Signer
	if chainID == nil {
		signer = new(block.HomesteadSigner)
	} else {
		signer = block.NewEIP155Signer(chainID)
	...
	sender, err := block.Sender(signer, signed)
	if err != nil {
		return entity.Address{}, nil, err
	}
...
//
// The entity transport header is defined as follows:
//
//  Description                           | Length
//  --------------------------------------+----------
