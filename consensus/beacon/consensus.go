...
var (
	beaconDifficulty = operationutils.Big0  // The default block difficulty in the beacon consensus
	beaconNonce      = block.EncodeNonce(0) // The default block nonce in the beacon consensus
)

// Various error messages to mark blocks invalid. These should be private to
// prevent engine specific errors from being referenced in the remainder of the
// codebase, inherently breaking if the engine is swapped out. Please put entity
// error block into the consensus package.
var (
...
func (beacon *Beacon) Author(header *block.Header) (entity.Address, error) {
...
func (beacon *Beacon) VerifyHeader(chain consensus.ChainHeaderReader, header *block.Header, seal bool) error {
...
func (beacon *Beacon) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*block.Header, seals []bool) (chan<- struct{}, <-chan error) {
	if !beacon.IsPoSHeader(headers[len(headers)-1]) {
		return beacon.ethone.VerifyHeaders(chain, headers, seals)
	}
	var (
		preHeaders  []*block.Header
		postHeaders []*block.Header
		preSeals    []bool
	)
...
func verifyTerminalPoWBlock(chain consensus.ChainHeaderReader, preHeaders []*block.Header) (int, error) {
...
//func (beacon *Beacon) VerifyUncles(chain consensus.ChainReader, block *block.Block) error {
...
func (beacon *Beacon) verifyHeader(chain consensus.ChainHeaderReader, header, parent *block.Header) error {
	...
	if header.UncleHash != block.EmptyUncleHash {
	...
	if diff := new(big.Int).Sub(header.Number, parent.Number); diff.Cmp(operationutils.Big1) != 0 {
...
func (beacon *Beacon) verifyHeaders(chain consensus.ChainHeaderReader, headers []*block.Header, ancestor *block.Header) (chan<- struct{}, <-chan error) {
	var (
		abort   = make(chan struct{})
		results = make(chan error, len(headers))
	)
...
func (beacon *Beacon) Prepare(chain consensus.ChainHeaderReader, header *block.Header) error {
...
func (beacon *Beacon) Finalize(chain consensus.ChainHeaderReader, header *block.Header, state *operationdb.OperationDB, txs []*block.Transaction, uncles []*block.Header) {
...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *block.Header, state *operationdb.OperationDB, txs []*block.Transaction, uncles []*block.Header, receipts []*block.Receipt) (*block.Block, error) {
	...
	return block.NewBlock(header, txs, receipts, trie.NewStackTrie(nil)), nil
}
...
func (beacon *Beacon) Seal(chain consensus.ChainHeaderReader, block *block.Block, results chan<- *block.Block, stop <-chan struct{}) error {
	if !beacon.IsPoSHeader(block.Header()) {
...
func (beacon *Beacon) SealHash(header *block.Header) entity.Hash {
...
func (beacon *Beacon) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *block.Header) *big.Int {
...
func (beacon *Beacon) IsPoSHeader(header *block.Header) bool {
...
func IsTTDReached(chain consensus.ChainHeaderReader, parentHash entity.Hash, number uint64) (bool, error) {
