...
import (
	...
	"github.com/radiation-octopus/octopus-blockchain/cmd/utils"
	"github.com/radiation-octopus/octopus-blockchain/console/prompt"
...
func confirmAndRemoveDB(database string, kind string) {
	...
	default:
		...
		log.Info("Database successfully deleted", "path", database, "elapsed", entity.PrettyDuration(time.Since(start)))
	}
}
...
//			log.Info("Iterating the database", "at", fmt.Sprintf("%#x", k), "elapsed", entity.PrettyDuration(time.Since(startTime)))
//			lastLog = time.Now()
//		}
//	}
...
//}
//
//func showLeveldbStats(db typedb.KeyValueStater) {
...
//	defer db.Close()
//
//	key, err := entity.ParseHexOrString(ctx.Args().Get(0))
//	if err != nil {
...
//		return err
//	}
//	stRoot := entity.BytesToHash(root)
//	if ctx.NArg() >= 2 {
...
//		}
//	}
//	theTrie, err := trie.New(entity.Hash{}, stRoot, trie.NewDatabase(db))
//	if err != nil {
//		return err
//	}
//	var count int64
...
//	iter typedb.Iterator
//}
//
//func (iter *preimageIterator) Next() (byte, []byte, []byte, bool) {
//	for iter.iter.Next() {
//		key := iter.iter.Key()
//		if bytes.HasPrefix(key, rawdb.PreimagePrefix) && len(key) == (len(rawdb.PreimagePrefix)+entity.HashLength) {
...
//	init    bool
//	account typedb.Iterator
//	storage typedb.Iterator
//}
//
...
//		if bytes.HasPrefix(key, rawdb.SnapshotAccountPrefix) && len(key) == (len(rawdb.SnapshotAccountPrefix)+entity.HashLength) {
//			return utils.OpBatchAdd, key, iter.account.Value(), true
//		}
//	}
//	for iter.storage.Next() {
//		key := iter.storage.Key()
//		if bytes.HasPrefix(key, rawdb.SnapshotStoragePrefix) && len(key) == (len(rawdb.SnapshotStoragePrefix)+2*entity.HashLength) {
...
//var chainExporters = map[string]func(db typedb.Database) utils.ChainDataIterator{
//	"preimage": func(db typedb.Database) utils.ChainDataIterator {
//		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
//		return &preimageIterator{iter: iter}
//	},
//	"snapshot": func(db typedb.Database) utils.ChainDataIterator {
...
//		}
//		return fmt.Errorf("invalid data type %s, supported block: %s", kind, strings.Join(kinds, ", "))
//	}
//	var (
//		stack, _  = makeConfigNode(ctx)
//		interrupt = make(chan os.Signal, 1)
...
//	start := time.Now()
//	if err := db.MigrateTable("receipts", block.ConvertLegacyStoredReceipts); err != nil {
//		return err
//	}
...
func dbHasLegacyReceipts(db typedb.Database, firstIdx uint64) (bool, uint64, error) {
	// Check first block for legacy receipt format
	numAncients, err := db.Ancients()
	if err != nil {
		return false, 0, err
	...
	legacy, err = block.IsLegacyStoredReceipts(first)
