...
import (
	...
	"github.com/radiation-octopus/octopus-blockchain/internal/debug"
...
func StartNode(ctx *cli.Context, stack *node.Node, isConsole bool) {
	...
	go func() {
		...
		minFreeDiskSpace := 2 * octconfig.Defaults.TrieDirtyCache // Default 2 * 256Mb
...
func monitorFreeDiskSpace(sigc chan os.Signal, path string, freeDiskSpaceCritical uint64) {
	for {
		...
		if freeSpace < freeDiskSpaceCritical {
			log.Error("Low disk space. Gracefully shutting down Geth to prevent database corruption.", "available", utils.StorageSize(freeSpace))
			sigc <- syscall.SIGTERM
			break
		} else if freeSpace < 2*freeDiskSpaceCritical {
			log.Warn("Disk space is running low. Geth will shutdown if disk space runs below critical level.", "available", utils.StorageSize(freeSpace), "critical_level", utils.StorageSize(freeDiskSpaceCritical))
		}
...
func ImportChain(chain *blockchain.BlockChain, fn string) error {
	...
	blocks := make(block.Blocks, importBatchSize)
	n := 0
	for batch := 0; ; batch++ {
...
func missingBlocks(chain *blockchain.BlockChain, blocks []*block.Block) []*block.Block {
...
func ImportPreimages(db typedb.Database, fn string) error {
	...
	for {
		...
		preimages[crypto.Keccak256Hash(blob)] = utils.CopyBytes(blob)
...
// It's a part of the deprecated functionality, should be removed in the future.
func ExportPreimages(db typedb.Database, fn string) error {
...
type exportHeader struct {
	Magic    string // Always set to 'gtypedbdump' for disambiguation
	Version  uint64
	Kind     string
	UnixTime uint64
}

const exportMagic = "gtypedbdump"
const (
	OpBatchAdd = 0
	OpBatchDel = 1
)
...
func ImportLDBData(db typedb.Database, f string, startIndex int64, interrupt chan struct{}) error {
	...
	log.Info("Importing data", "file", f, "type", header.Kind, "data age",
		entity.PrettyDuration(time.Since(time.Unix(int64(header.UnixTime), 0))))
	...
	for {
		...
		if count%1000 == 0 {
			...
			case <-interrupt:
				...
				log.Info("External data import interrupted", "file", f, "count", count, "elapsed", entity.PrettyDuration(time.Since(start)))
				return nil
			default:
			}
		}
		if count%1000 == 0 && time.Since(logged) > 8*time.Second {
			log.Info("Importing external data", "file", f, "count", count, "elapsed", entity.PrettyDuration(time.Since(start)))
			logged = time.Now()
		}
		count += 1
	}
	...
	log.Info("Imported chain data", "file", f, "count", count,
		"elapsed", entity.PrettyDuration(time.Since(start)))
	return nil
}
		...
		if count%1000 == 0 {
			...
			case <-interrupt:
				log.Info("Chain data exporting interrupted", "file", fn,
					"kind", kind, "count", count, "elapsed", entity.PrettyDuration(time.Since(start)))
				return nil
			default:
			}
			if time.Since(logged) > 8*time.Second {
				log.Info("Exporting chain data", "file", fn, "kind", kind,
					"count", count, "elapsed", entity.PrettyDuration(time.Since(start)))
				logged = time.Now()
			}
		}
		count++
