...
import (
	...
	"github.com/radiation-octopus/octopus-blockchain/typedb/remotedb"
	"math"
	"math/big"
	"os"
	"path/filepath"
	...
	"github.com/radiation-octopus/octopus-blockchain/entity/fdlimit"
	"github.com/radiation-octopus/octopus-blockchain/internal/ethapi"
	"github.com/radiation-octopus/octopus-blockchain/internal/flags"
	"github.com/radiation-octopus/octopus-blockchain/log"
	"github.com/radiation-octopus/octopus-blockchain/metrics"
	"github.com/radiation-octopus/octopus-blockchain/metrics/exp"
	"github.com/radiation-octopus/octopus-blockchain/metrics/influxdb"
...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		...
		Value:    octconfig.Defaults.NetworkId,
		Category: flags.EthCategory,
	}
	...
	defaultSyncMode = octconfig.Defaults.SyncMode
	...
	TxLookupLimitFlag = &cli.Uint64Flag{
		...
		Value:    octconfig.Defaults.TxLookupLimit,
		Category: flags.EthCategory,
	}
	...
	LightServeFlag = &cli.IntFlag{
		...
		Value:    octconfig.Defaults.LightServ,
		Category: flags.LightCategory,
	}
	LightIngressFlag = &cli.IntFlag{
		Name:     "light.ingress",
		Usage:    "Incoming bandwidth limit for serving light clients (kilobytes/sec, 0 = unlimited)",
		Value:    octconfig.Defaults.LightIngress,
		Category: flags.LightCategory,
	}
	LightEgressFlag = &cli.IntFlag{
		Name:     "light.egress",
		Usage:    "Outgoing bandwidth limit for serving light clients (kilobytes/sec, 0 = unlimited)",
		Value:    octconfig.Defaults.LightEgress,
		Category: flags.LightCategory,
	}
	LightMaxPeersFlag = &cli.IntFlag{
		Name:     "light.maxpeers",
		Usage:    "Maximum number of light clients to serve, or light servers to attach to",
		Value:    octconfig.Defaults.LightPeers,
		Category: flags.LightCategory,
	}
	UltraLightServersFlag = &cli.StringFlag{
		Name:     "ulc.servers",
		Usage:    "List of trusted ultra-light servers",
		Value:    strings.Join(octconfig.Defaults.UltraLightServers, ","),
		Category: flags.LightCategory,
	}
	UltraLightFractionFlag = &cli.IntFlag{
		Name:     "ulc.fraction",
		Usage:    "Minimum % of trusted ultra-light servers required to announce a new head",
		Value:    octconfig.Defaults.UltraLightFraction,
		Category: flags.LightCategory,
	}
	...
	EthashCachesInMemoryFlag = &cli.IntFlag{
		...
		Value:    octconfig.Defaults.Octell.CachesInMem,
		Category: flags.EthashCategory,
	}
	EthashCachesOnDiskFlag = &cli.IntFlag{
		Name:     "ethash.cachesondisk",
		Usage:    "Number of recent ethash caches to keep on disk (16MB each)",
		Value:    octconfig.Defaults.Octell.CachesOnDisk,
		Category: flags.EthashCategory,
	}
	...
	EthashDatasetDirFlag = &flags.DirectoryFlag{
		...
		Value:    flags.DirectoryString(octconfig.Defaults.Octell.DatasetDir),
		Category: flags.EthashCategory,
	}
	EthashDatasetsInMemoryFlag = &cli.IntFlag{
		Name:     "ethash.dagsinmem",
		Usage:    "Number of recent ethash mining DAGs to keep in memory (1+GB each)",
		Value:    octconfig.Defaults.Octell.DatasetsInMem,
		Category: flags.EthashCategory,
	}
	EthashDatasetsOnDiskFlag = &cli.IntFlag{
		Name:     "ethash.dagsondisk",
		Usage:    "Number of recent ethash mining DAGs to keep on disk (1+GB each)",
		Value:    octconfig.Defaults.Octell.DatasetsOnDisk,
		Category: flags.EthashCategory,
	}
	...
	TxPoolJournalFlag = &cli.StringFlag{
		...
		Value:    blockchainconfig.DefaultTxPoolConfig.Journal,
		Category: flags.TxPoolCategory,
	}
	TxPoolRejournalFlag = &cli.DurationFlag{
		Name:     "txpool.rejournal",
		Usage:    "Time interval to regenerate the local transaction journal",
		Value:    blockchainconfig.DefaultTxPoolConfig.Rejournal,
		Category: flags.TxPoolCategory,
	}
	TxPoolPriceLimitFlag = &cli.Uint64Flag{
		Name:     "txpool.pricelimit",
		Usage:    "Minimum gas price limit to enforce for acceptance into the pool",
		Value:    octconfig.Defaults.TxPool.PriceLimit,
		Category: flags.TxPoolCategory,
	}
	TxPoolPriceBumpFlag = &cli.Uint64Flag{
		Name:     "txpool.pricebump",
		Usage:    "Price bump percentage to replace an already existing transaction",
		Value:    octconfig.Defaults.TxPool.PriceBump,
		Category: flags.TxPoolCategory,
	}
	TxPoolAccountSlotsFlag = &cli.Uint64Flag{
		Name:     "txpool.accountslots",
		Usage:    "Minimum number of executable transaction slots guaranteed per account",
		Value:    octconfig.Defaults.TxPool.AccountSlots,
		Category: flags.TxPoolCategory,
	}
	TxPoolGlobalSlotsFlag = &cli.Uint64Flag{
		Name:     "txpool.globalslots",
		Usage:    "Maximum number of executable transaction slots for all accounts",
		Value:    octconfig.Defaults.TxPool.GlobalSlots,
		Category: flags.TxPoolCategory,
	}
	TxPoolAccountQueueFlag = &cli.Uint64Flag{
		Name:     "txpool.accountqueue",
		Usage:    "Maximum number of non-executable transaction slots permitted per account",
		Value:    octconfig.Defaults.TxPool.AccountQueue,
		Category: flags.TxPoolCategory,
	}
	TxPoolGlobalQueueFlag = &cli.Uint64Flag{
		Name:     "txpool.globalqueue",
		Usage:    "Maximum number of non-executable transaction slots for all accounts",
		Value:    octconfig.Defaults.TxPool.GlobalQueue,
		Category: flags.TxPoolCategory,
	}
	TxPoolLifetimeFlag = &cli.DurationFlag{
		Name:     "txpool.lifetime",
		Usage:    "Maximum amount of time non-executable transaction are queued",
		Value:    octconfig.Defaults.TxPool.Lifetime,
		Category: flags.TxPoolCategory,
	}
	...
	CacheTrieJournalFlag = &cli.StringFlag{
		...
		Value:    octconfig.Defaults.TrieCleanCacheJournal,
		Category: flags.PerfCategory,
	}
	CacheTrieRejournalFlag = &cli.DurationFlag{
		Name:     "cache.trie.rejournal",
		Usage:    "Time interval to regenerate the trie cache journal",
		Value:    octconfig.Defaults.TrieCleanCacheRejournal,
		Category: flags.PerfCategory,
	}
	...
	MinerGasLimitFlag = &cli.Uint64Flag{
		...
		Value:    octconfig.Defaults.Miner.GasCeil,
		Category: flags.MinerCategory,
	}
	MinerGasPriceFlag = &flags.BigFlag{
		Name:     "miner.gasprice",
		Usage:    "Minimum gas price for mining a transaction",
		Value:    octconfig.Defaults.Miner.GasPrice,
		Category: flags.MinerCategory,
	}
	...
	MinerRecommitIntervalFlag = &cli.DurationFlag{
		...
		Value:    octconfig.Defaults.Miner.Recommit,
		Category: flags.MinerCategory,
	}
	...
	RPCGlobalGasCapFlag = &cli.Uint64Flag{
		...
		Value:    octconfig.Defaults.RPCGasCap,
		Category: flags.APICategory,
	}
	RPCGlobalEVMTimeoutFlag = &cli.DurationFlag{
		Name:     "rpc.evmtimeout",
		Usage:    "Sets a timeout used for eth_call (0=infinite)",
		Value:    octconfig.Defaults.RPCEVMTimeout,
		Category: flags.APICategory,
	}
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in ether) that can be sent via the RPC APIs (0 = no cap)",
		Value:    octconfig.Defaults.RPCTxFeeCap,
		Category: flags.APICategory,
	}
	...
	GpoBlocksFlag = &cli.IntFlag{
		...
		Value:    octconfig.Defaults.GPO.Blocks,
		Category: flags.GasPriceCategory,
	}
	GpoPercentileFlag = &cli.IntFlag{
		Name:     "gpo.percentile",
		Usage:    "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value:    octconfig.Defaults.GPO.Percentile,
		Category: flags.GasPriceCategory,
	}
	GpoMaxGasPriceFlag = &cli.Int64Flag{
		Name:     "gpo.maxprice",
		Usage:    "Maximum transaction priority fee (or gasprice before London fork) to be recommended by gpo",
		Value:    octconfig.Defaults.GPO.MaxPrice.Int64(),
		Category: flags.GasPriceCategory,
	}
	GpoIgnoreGasPriceFlag = &cli.Int64Flag{
		Name:     "gpo.ignoreprice",
		Usage:    "Gas price below which gpo will ignore transactions",
		Value:    octconfig.Defaults.GPO.IgnorePrice.Int64(),
		Category: flags.GasPriceCategory,
	}
...
func setLes(ctx *cli.Context, cfg *octconfig.Config) {
	...
	if cfg.UltraLightFraction <= 0 && cfg.UltraLightFraction > 100 {
		log.Error("Ultra light fraction is invalid", "had", cfg.UltraLightFraction, "updated", octconfig.Defaults.UltraLightFraction)
		cfg.UltraLightFraction = octconfig.Defaults.UltraLightFraction
	}
...
func MakeAddress(ks *accounts.KeyStore, account string) (accounts.Account, error) {
	// If the specified account is a valid address, return it
	if entity.IsHexAddress(account) {
		return accounts.Account{Address: entity.HexToAddress(account)}, nil
	}
...
func setEtherbase(ctx *cli.Context, ks *accounts.KeyStore, cfg *octconfig.Config) {
	...
	if etherbase != "" {
		if ks != nil {
			...
			cfg.Miner.Octerbase = account.Address
		} else {
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(RopstenFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		...
		if entity.FileExist(legacyPath) {
...
func setTxPool(ctx *cli.Context, cfg *blockchainconfig.TxPoolConfig) {
	if ctx.IsSet(TxPoolLocalsFlag.Name) {
		locals := strings.Split(ctx.String(TxPoolLocalsFlag.Name), ",")
		for _, account := range locals {
			if trimmed := strings.TrimSpace(account); !entity.IsHexAddress(trimmed) {
				Fatalf("Invalid account in --txpool.locals: %s", trimmed)
			} else {
				cfg.Locals = append(cfg.Locals, entity.HexToAddress(account))
			}
		}
	}
...
func setEthash(ctx *cli.Context, cfg *octconfig.Config) {
	if ctx.IsSet(EthashCacheDirFlag.Name) {
		cfg.Octell.CacheDir = ctx.String(EthashCacheDirFlag.Name)
	}
	if ctx.IsSet(EthashDatasetDirFlag.Name) {
		cfg.Octell.DatasetDir = ctx.String(EthashDatasetDirFlag.Name)
	}
	if ctx.IsSet(EthashCachesInMemoryFlag.Name) {
		cfg.Octell.CachesInMem = ctx.Int(EthashCachesInMemoryFlag.Name)
	}
	if ctx.IsSet(EthashCachesOnDiskFlag.Name) {
		cfg.Octell.CachesOnDisk = ctx.Int(EthashCachesOnDiskFlag.Name)
	}
	if ctx.IsSet(EthashCachesLockMmapFlag.Name) {
		cfg.Octell.CachesLockMmap = ctx.Bool(EthashCachesLockMmapFlag.Name)
	}
	if ctx.IsSet(EthashDatasetsInMemoryFlag.Name) {
		cfg.Octell.DatasetsInMem = ctx.Int(EthashDatasetsInMemoryFlag.Name)
	}
	if ctx.IsSet(EthashDatasetsOnDiskFlag.Name) {
		cfg.Octell.DatasetsOnDisk = ctx.Int(EthashDatasetsOnDiskFlag.Name)
	}
	if ctx.IsSet(EthashDatasetsLockMmapFlag.Name) {
		cfg.Octell.DatasetsLockMmap = ctx.Bool(EthashDatasetsLockMmapFlag.Name)
	}
}

func setMiner(ctx *cli.Context, cfg *blockchainconfig.Config) {
...
func setRequiredBlocks(ctx *cli.Context, cfg *octconfig.Config) {
	...
	cfg.RequiredBlocks = make(map[uint64]entity.Hash)
...
// Setoctconfig applies eth-related command line flags to the config.
func Setoctconfig(ctx *cli.Context, stack *node.Node, cfg *octconfig.Config) {
	...
	var ks *accounts.KeyStore
	if keystores := stack.AccountManager().Backends(accounts.KeyStoreType); len(keystores) > 0 {
		ks = keystores[0].(*accounts.KeyStore)
	}
	...
	case ctx.Bool(MainnetFlag.Name):
		...
		cfg.Genesis = genesis.DefaultGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.MainnetGenesisHash)
	case ctx.Bool(RopstenFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 3
		}
		cfg.Genesis = genesis.DefaultRopstenGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.RopstenGenesisHash)
	case ctx.Bool(SepoliaFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 11155111
		}
		//cfg.Genesis = genesis.DefaultSepoliaGenesisBlock()
	...
	case ctx.Bool(RinkebyFlag.Name):
		...
		//cfg.Genesis = genesis.DefaultRinkebyGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.RinkebyGenesisHash)
	case ctx.Bool(GoerliFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 5
		}
		//cfg.Genesis = genesis.DefaultGoerliGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.GoerliGenesisHash)
	case ctx.Bool(KilnFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337802
		}
		//cfg.Genesis = genesis.DefaultKilnGenesisBlock()
	...
	case ctx.Bool(DeveloperFlag.Name):
		...
		if cfg.Miner.Octerbase != (entity.Address{}) {
			developer = accounts.Account{Address: cfg.Miner.Octerbase}
		...
		//cfg.Genesis = genesis.DeveloperGenesisBlock(uint64(ctx.Int(DeveloperPeriodFlag.Name)), ctx.Uint64(DeveloperGasLimitFlag.Name), developer.Address)
		if ctx.IsSet(DataDirFlag.Name) {
			// If datadir doesn't exist we need to open db in write-mode
			// so leveldb can create files.
			readonly := true
			if !entity.FileExist(stack.ResolvePath("chaindata")) {
				readonly = false
			}
			// Check if we have an already initialized chain and fall back to
			// that if so. Otherwise we need to generate a new genesis spec.
			chaindb := MakeChainDatabase(ctx, stack, readonly)
			if rawdb.ReadCanonicalHash(chaindb, 0) != (entity.Hash{}) {
...
// no URLs are set.
func SetDNSDiscoveryDefaults(cfg *octconfig.Config, genesis entity.Hash) {
...
func RegisterEthService(stack *node.Node, cfg *octconfig.Config) (ethapi.Backend, *oct.Octopus) {
	//if cfg.SyncMode == downloader.LightSync {
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node, readonly bool) typedb.Database {
	var (
...
func MakeGenesis(ctx *cli.Context) *genesis.Genesis {
	...
	//case ctx.Bool(SepoliaFlag.Name):
	//	genesis = genesis.DefaultSepoliaGenesisBlock()
	//case ctx.Bool(RinkebyFlag.Name):
	//	genesis = genesis.DefaultRinkebyGenesisBlock()
	//case ctx.Bool(GoerliFlag.Name):
	//	genesis = genesis.DefaultGoerliGenesisBlock()
	//case ctx.Bool(KilnFlag.Name):
	//	genesis = genesis.DefaultKilnGenesisBlock()
	//case ctx.Bool(DeveloperFlag.Name):
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *blockchain.BlockChain, chainDb typedb.Database) {
	var err error
	chainDb = MakeChainDatabase(ctx, stack, false) // TODO(rjl493456442) support read-only database
	//config, _, err := genesis.SetupGenesisBlock(chainDb, MakeGenesis(ctx))
	if err != nil {
		Fatalf("%v", err)
	}

	//var engine consensus.Engine
	//ethashConf := octconfig.Defaults.Octell
	//if ctx.Bool(FakePoWFlag.Name) {
	//	ethashConf.PowMode = ethash.ModeFake
	//}
	//engine = octconfig.CreateConsensusEngine(stack, config, &ethashConf, nil, false, chainDb)
	if gcmode := ctx.String(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
		Fatalf("--%s must be either 'full' or 'archive'", GCModeFlag.Name)
	}
	cache := &blockchain.CacheConfig{
		TrieCleanLimit:      octconfig.Defaults.TrieCleanCache,
		TrieCleanNoPrefetch: ctx.Bool(CacheNoPrefetchFlag.Name),
		TrieDirtyLimit:      octconfig.Defaults.TrieDirtyCache,
		TrieDirtyDisabled:   ctx.String(GCModeFlag.Name) == "archive",
		TrieTimeLimit:       octconfig.Defaults.TrieTimeout,
		SnapshotLimit:       octconfig.Defaults.SnapshotCache,
	...
	//chain, err = blockchain.NewBlockChain(chainDb, cache, config, engine, vmcfg, nil)
	if err != nil {
