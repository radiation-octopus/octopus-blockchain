...
func (w *wizard) readAddress() *entity.Address {
	for {
		text := promptInput("> 0x")
		if text = strings.TrimSpace(text); text == "" {
			return nil
		...
		address := entity.BigToAddress(bigaddr)
		return &address
	}
}
...
// value is returned.
func (w *wizard) readDefaultAddress(def entity.Address) entity.Address {
	for {
		...
		bigaddr, _ := new(big.Int).SetString(text, 16)
		return entity.BigToAddress(bigaddr)
	}
}
