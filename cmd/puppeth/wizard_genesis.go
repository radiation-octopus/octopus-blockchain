...
func (w *wizard) makeGenesis() {
	...
	case choice == "" || choice == "2":
		...
		genesis.ExtraData = make([]byte, 32+len(signers)*entity.AddressLength+65)
		for i, signer := range signers {
			copy(genesis.ExtraData[32+i*entity.AddressLength:], signer[:])
		}
	...
	for {
		...
		if address := w.readAddress(); address != nil {
			genesis.Alloc[*address] = g.GenesisAccount{
	...
	if w.readDefaultYesNo(true) {
		...
		for i := int64(0); i < 256; i++ {
			genesis.Alloc[entity.BigToAddress(big.NewInt(i))] = g.GenesisAccount{Balance: big.NewInt(1)}
		}
	}
