...
func (w *wizard) deployFaucet() {
	...
	if err != nil {
		infos = &faucetInfos{
			node:    &nodeInfos{port: 58888, peersTotal: 25},
			port:    80,
			host:    client.server,
			amount:  1,
			minutes: 1440,
	...
	if infos.node.keyJSON != "" {
		if key, err := accounts.DecryptKey([]byte(infos.node.keyJSON), infos.node.keyPass); err != nil {
	...
	for i := 0; i < 3 && infos.node.keyJSON == ""; i++ {
		...
		if _, err := accounts.DecryptKey([]byte(infos.node.keyJSON), infos.node.keyPass); err != nil {
