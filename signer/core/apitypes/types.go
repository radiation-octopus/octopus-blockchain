...
// entity.MixedcaseAddress in From and To
type SendTxArgs struct {
	From                 entity.MixedcaseAddress  `json:"from"`
	To                   *entity.MixedcaseAddress `json:"to"`
	...
	//AccessList *block.AccessList `json:"accessList,omitempty"`
...
func (args *SendTxArgs) ToTransaction() *block.Transaction {
...
func (typedData *TypedData) EncodePrimitiveValue(encType string, encValue interface{}, depth int) ([]byte, error) {
	...
	case "address":
		...
		if !ok || !entity.IsHexAddress(stringValue) {
			return nil, dataMismatchError(encType, encValue)
		}
		retval := make([]byte, 32)
		copy(retval[12:], entity.HexToAddress(stringValue).Bytes())
		return retval, nil
	case "bool":
		...
		if boolValue {
			return math.PaddedBigBytes(operationutils.Big1, 32), nil
		}
		return math.PaddedBigBytes(operationutils.Big0, 32), nil
	case "string":
...
func formatPrimitiveValue(encType string, encValue interface{}) (string, error) {
	...
	case "address":
		if stringValue, ok := encValue.(string); !ok {
			...
			return entity.HexToAddress(stringValue).String(), nil
		}
	case "bool":
