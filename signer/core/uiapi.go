...
func fetchKeystore(am *accounts.Manager) *accounts.KeyStore {
	return am.Backends(accounts.KeyStoreType)[0].(*accounts.KeyStore)
}
...
func (s *UIServerAPI) Export(ctx context.Context, addr entity.Address) (json.RawMessage, error) {
	// Look up the wallet containing the requested signer
	wallet, err := s.am.Find(accounts.Account{Address: addr})
	if err != nil {
		return nil, err
	}
	if wallet.URL().Scheme != accounts.KeyStoreScheme {
...
func (api *UIServerAPI) Import(ctx context.Context, keyJSON json.RawMessage, oldPassphrase, newPassphrase string) (accounts.Account, error) {
	...
	return be[0].(*accounts.KeyStore).Import(keyJSON, oldPassphrase, newPassphrase)
}
...
// the user
func (api *UIServerAPI) New(ctx context.Context) (entity.Address, error) {
