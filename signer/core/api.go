...
import (
	...
	"github.com/radiation-octopus/octopus-blockchain/accounts/usbwallet"
	"math/big"
	"os"
	"reflect"
	...
	"github.com/radiation-octopus/octopus-blockchain/signer/core/apitypes"
	"github.com/radiation-octopus/octopus-blockchain/signer/storage"
)
...
type ExternalAPI interface {
	...
	New(ctx context.Context) (entity.Address, error)
	...
	SignData(ctx context.Context, contentType string, addr entity.MixedcaseAddress, data interface{}) (hexutil.Bytes, error)
	// SignTypedData - request to sign the given structured data (plus prefix)
	SignTypedData(ctx context.Context, addr entity.MixedcaseAddress, data apitypes.TypedData) (hexutil.Bytes, error)
	// EcRecover - recover public key from given message and signature
	EcRecover(ctx context.Context, data hexutil.Bytes, sig hexutil.Bytes) (entity.Address, error)
	// Version info about the APIs
	Version(ctx context.Context) (string, error)
	// SignGnosisSafeTransaction signs/confirms a gnosis-safe multisig transaction
	SignGnosisSafeTx(ctx context.Context, signerAddress entity.MixedcaseAddress, gnosisTx GnosisSafeTx, methodSelector *string) (*GnosisSafeTx, error)
}
...
func StartClefAccountManager(ksLocation string, nousb, lightKDF bool, scpath string) *accounts.Manager {
	...
	if lightKDF {
		n, p = accounts.LightScryptN, accounts.LightScryptP
	}
	// support password based accounts
	if len(ksLocation) > 0 {
		backends = append(backends, accounts.NewKeyStore(ksLocation, n, p))
	}
	if !nousb {
...
type (
	...
	SignDataRequest struct {
		...
		Address     entity.MixedcaseAddress   `json:"address"`
...
// multiple accounts.
func (api *SignerAPI) List(ctx context.Context) ([]entity.Address, error) {
...
func (api *SignerAPI) New(ctx context.Context) (entity.Address, error) {
	if be := api.am.Backends(accounts.KeyStoreType); len(be) == 0 {
		return entity.Address{}, errors.New("password based accounts not supported")
	}
	if resp, err := api.UI.ApproveNewAccount(&NewAccountRequest{MetadataFromContext(ctx)}); err != nil {
		return entity.Address{}, err
	} else if !resp.Approved {
		return entity.Address{}, ErrRequestDenied
	}
...
func (api *SignerAPI) newAccount() (entity.Address, error) {
	be := api.am.Backends(accounts.KeyStoreType)
	if len(be) == 0 {
		return entity.Address{}, errors.New("password based accounts not supported")
	}
	...
	for i := 0; i < 3; i++ {
		...
		if pwErr := ValidatePasswordFormat(resp.Text); pwErr != nil {
			...
			// No error
			acc, err := be[0].(*accounts.KeyStore).NewAccount(resp.Text)
	...
	return entity.Address{}, errors.New("account creation failed")
}
...
func (api *SignerAPI) lookupPassword(address entity.Address) (string, error) {
	return api.credentials.Get(address.Hex())
}

func (api *SignerAPI) lookupOrQueryPassword(address entity.Address, title, prompt string) (string, error) {
...
func (api *SignerAPI) SignGnosisSafeTx(ctx context.Context, signerAddress entity.MixedcaseAddress, gnosisTx GnosisSafeTx, methodSelector *string) (*GnosisSafeTx, error) {
	...
	checkSummedSender, _ := entity.NewMixedcaseAddressFromString(signerAddress.Address().Hex())

	gnosisTx.Signature = signature
	gnosisTx.SafeTxHash = entity.BytesToHash(preimage)
