...
// reassembly.
func makeChain(n int, seed byte, parent *block.Block, empty bool) ([]*block.Block, []block.Receipts) {
	blocks, receipts := blockchain.GenerateChain(params.TestChainConfig, parent, ethash.NewFaker(), testdb, n, func(i int, block *core.BlockGen) {
		block.SetCoinbase(entity.Address{seed})
		// Add one tx to every secondblock
		if !empty && i%2 == 0 {
			signer := block.MakeSigner(params.TestChainConfig, block.Number())
			tx, err := block.SignTx(block.NewTransaction(block.TxNonce(testAddress), entity.Address{seed}, big.NewInt(1000), params.TxGas, block.BaseFee(), nil), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
...
func (chain *chainData) headers() []*block.Header {
	hdrs := make([]*block.Header, len(chain.blocks))
...
func dummyPeer(id string) *peerConnection {
	p := &peerConnection{
		id:      id,
		lacking: make(map[entity.Hash]struct{}),
	}
	return p
}
...
func XTestDelivery(t *testing.T) {
	...
	go func() {
		...
		for {
			...
			if f != nil {
				var (
					emptyList []*block.Header
					txset     [][]*block.Transaction
					uncleset  [][]*block.Header
				)
				...
				var (
					txsHashes   = make([]entity.Hash, len(txset))
					uncleHashes = make([]entity.Hash, len(uncleset))
				)
				hasher := trie.NewStackTrie(nil)
				for i, txs := range txset {
					txsHashes[i] = block.DeriveSha(block.Transactions(txs), hasher)
				}
				for i, uncles := range uncleset {
					uncleHashes[i] = block.CalcUncleHash(uncles)
				}
	...
	go func() {
		...
		for {
			...
			if f != nil {
				var rcs [][]*block.Receipt
				for _, hdr := range f.Headers {
					rcs = append(rcs, world.getReceipts(hdr.Number.Uint64()))
				}
				hasher := trie.NewStackTrie(nil)
				hashes := make([]entity.Hash, len(rcs))
...
func (n *network) getTransactions(blocknum uint64) block.Transactions {
	index := blocknum - uint64(n.offset)
	return n.chain[index].Transactions()
}
func (n *network) getReceipts(blocknum uint64) block.Receipts {
	index := blocknum - uint64(n.offset)
...
func (n *network) headers(from int) []*block.Header {
	numHeaders := 128
