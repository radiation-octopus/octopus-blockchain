...
type beaconBackfiller struct {
	...
	filled     *block.Header // Last header filled by the last terminated sync loop
...
func (b *beaconBackfiller) suspend() *block.Header {
...
func (b *beaconBackfiller) resume() {
	...
	go func() {
		...
		if err := b.downloader.synchronise("", entity.Hash{}, nil, nil, mode, true, b.started); err != nil {
...
func (d *Downloader) BeaconSync(mode SyncMode, head *block.Header) error {
...
func (d *Downloader) BeaconExtend(mode SyncMode, head *block.Header) error {
...
// Internally backfilling and state sync is done the same way, but the header
// retrieval and scheduling is replaced.
func (d *Downloader) beaconSync(mode SyncMode, head *block.Header, force bool) error {
...
// findBeaconAncestor tries to locate the entity ancestor link of the local chain
...
func (d *Downloader) fetchBeaconHeaders(from uint64) error {
	...
	for {
		...
		var (
			headers = make([]*block.Header, 0, maxHeadersProcess)
			hashes  = make([]entity.Hash, 0, maxHeadersProcess)
		)
