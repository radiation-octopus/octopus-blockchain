...
type subchain struct {
	...
	Next entity.Hash // Block hash of the next oldest header in the subchain
}
...
type headUpdate struct {
	header *block.Header // Header to update the sync target to
...
type headerResponse struct {
	...
	headers []*block.Header // Chain of headers
}
...
type skeleton struct {
	db     typedb.Database // Database backing the skeleton
	...
	scratchSpace  []*block.Header // Scratch space to accumulate headers in (first = recent)
...
func newSkeleton(db typedb.Database, peers *peerSet, drop peerDropFn, filler backfiller) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		peers:      peers,
...
func (s *skeleton) Sync(head *block.Header, force bool) error {
...
func (s *skeleton) sync(head *block.Header) (*block.Header, error) {
	...
	s.scratchSpace = make([]*block.Header, scratchHeaders)
...
func (s *skeleton) initSync(head *block.Header) {
...
func (s *skeleton) saveSyncStatus(db typedb.KeyValueWriter) {
...
func (s *skeleton) processNewHead(head *block.Header, force bool) bool {
...
func (s *skeleton) processResponse(res *headerResponse) (linked bool, merged bool) {
	...
	if time.Since(s.logged) > 8*time.Second || left == 0 {
		...
		if s.pulled == 0 {
			...
			log.Info("Syncing beacon headers", "downloaded", s.pulled, "left", left, "eta", entity.PrettyDuration(eta))
		}
	}
...
func (s *skeleton) cleanStales(filled *block.Header) error {
...
func (s *skeleton) Bounds() (head *block.Header, tail *block.Header, err error) {
...
func (s *skeleton) Header(number uint64) *block.Header {
