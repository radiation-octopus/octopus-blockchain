...
func (d *Downloader) fetchHeadersByHash(p *peerConnection, hash entity.Hash, amount int, skip int, reverse bool) ([]*block.Header, []entity.Hash, error) {
	...
	case res := <-resCh:
		...
		return *res.Res.(*eth.BlockHeadersPacket), res.Meta.([]entity.Hash), nil
	}
}
...
// handles all the cancellation, interruption and timeout mechanisms of a data
// retrieval to allow blocking API calls.
func (d *Downloader) fetchHeadersByNumber(p *peerConnection, number uint64, amount int, skip int, reverse bool) ([]*block.Header, []entity.Hash, error) {
	// Create the response sink and send the network request
	start := time.Now()
	resCh := make(chan *eth.Response)
