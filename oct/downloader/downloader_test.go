...
func newTesterWithNotification(t *testing.T, success func()) *downloadTester {
	...
	blockchain.GenesisBlockForTesting(db, testAddress, big.NewInt(1000000000000000))

	chain, err := blockchain.NewBlockChain(db, nil, params.TestChainConfig, ethash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
		panic(err)
	}
...
func (dl *downloadTester) newPeer(id string, version uint, blocks []*block.Block) *downloadTesterPeer {
	dl.lock.Lock()
	...
	peer := &downloadTesterPeer{
		...
		withholdHeaders: make(map[entity.Hash]struct{}),
	}
	dl.peers[id] = peer
...
type downloadTesterPeer struct {
	...
	withholdHeaders map[entity.Hash]struct{}
}

// Head constructs a function to retrieve a peer's current head hash
// and total difficulty.
func (dlp *downloadTesterPeer) Head() (entity.Hash, *big.Int) {
	head := dlp.chain.CurrentBlock()
	return head.Hash(), dlp.chain.GetTd(head.Hash(), head.NumberU64())
}

func unmarshalRlpHeaders(rlpdata []rlp.RawValue) []*block.Header {
	var headers = make([]*block.Header, len(rlpdata))
...
func (dlp *downloadTesterPeer) RequestHeadersByHash(origin entity.Hash, amount int, skip int, reverse bool, sink chan *eth.Response) (*eth.Request, error) {
	...
	hashes := make([]entity.Hash, len(headers))
	for i, header := range headers {
...
func (dlp *downloadTesterPeer) RequestBodies(hashes []entity.Hash, sink chan *eth.Response) (*eth.Request, error) {
	...
	var (
		txsHashes   = make([]entity.Hash, len(bodies))
		uncleHashes = make([]entity.Hash, len(bodies))
	)
	hasher := trie.NewStackTrie(nil)
	for i, body := range bodies {
		txsHashes[i] = block.DeriveSha(block.Transactions(body.Transactions), hasher)
		uncleHashes[i] = block.CalcUncleHash(body.Uncles)
	}
	req := &eth.Request{
		Peer: dlp.id,
	}
	res := &eth.Response{
		...
		Meta: [][]entity.Hash{txsHashes, uncleHashes},
		Time: 1,
		Done: make(chan error, 1), // Ignore the returned status
	}
	go func() {
...
func (dlp *downloadTesterPeer) RequestReceipts(hashes []entity.Hash, sink chan *eth.Response) (*eth.Request, error) {
	blobs := eth.ServiceGetReceiptsQuery(dlp.chain, hashes)

	receipts := make([][]*block.Receipt, len(blobs))
	for i, blob := range blobs {
		rlp.DecodeBytes(blob, &receipts[i])
	}
	hasher := trie.NewStackTrie(nil)
	hashes = make([]entity.Hash, len(receipts))
	for i, receipt := range receipts {
		hashes[i] = block.DeriveSha(block.Receipts(receipt), hasher)
	}
	req := &eth.Request{
		Peer: dlp.id,
	}
...
func (dlp *downloadTesterPeer) RequestAccountRange(id uint64, root, origin, limit entity.Hash, bytes uint64) error {
...
func (dlp *downloadTesterPeer) RequestStorageRanges(id uint64, root entity.Hash, accounts []entity.Hash, origin, limit []byte, bytes uint64) error {
	// Create the request and service it
...
func (dlp *downloadTesterPeer) RequestByteCodes(id uint64, hashes []entity.Hash, bytes uint64) error {
...
func (dlp *downloadTesterPeer) RequestTrieNodes(id uint64, root entity.Hash, paths []snap.TrieNodePathSet, bytes uint64) error {
...
// this test entity ancestor lookup should *not* be short circuited, and a full
...
func testBlockHeaderAttackerDropping(t *testing.T, protocol uint) {
	...
	for i, tt := range tests {
		...
		tester.downloader.synchroniseMock = func(string, entity.Hash) error { return tt.result }
