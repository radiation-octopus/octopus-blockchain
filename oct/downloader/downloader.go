...
var (
	...
	fullMaxForkAncestry  uint64 = entity.FullImmutabilityThreshold  // Maximum chain reorganisation (locally redeclared so tests can reduce it)
	lightMaxForkAncestry uint64 = entity.LightImmutabilityThreshold // Maximum chain reorganisation (locally redeclared so tests can reduce it)
...
var (
	...
	errNoAncestorFound         = errors.New("no entity ancestor found")
...
type Downloader struct {
	...
	stateDB typedb.Database // Database to state sync into (and deduplicate via)
	...
	// Status
	synchroniseMock func(id string, hash entity.Hash) error // Replacement for synchronise during testing
	synchronising   int32
	notified        int32
	committed       int32
	...
	// State sync
	pivotHeader *block.Header // Pivot block header to dynamically push the syncing state root
...
type LightChain interface {
	...
	HasHeader(entity.Hash, uint64) bool

	// GetHeaderByHash retrieves a header from the local chain.
	GetHeaderByHash(entity.Hash) *block.Header
...
type BlockChain interface {
	...
	HasBlock(entity.Hash, uint64) bool

	// HasFastBlock verifies a snap block's presence in the local chain.
	HasFastBlock(entity.Hash, uint64) bool

	// GetBlockByHash retrieves a block from the local chain.
	GetBlockByHash(entity.Hash) *block.Block
	...
	SnapSyncCommitHead(entity.Hash) error

	// InsertChain inserts a batch of blocks into the local chain.
	InsertChain(block.Blocks) (int, error)

	// InsertReceiptChain inserts a batch of receipts into the local chain.
	InsertReceiptChain(block.Blocks, []block.Receipts, uint64) (int, error)
...
func New(checkpoint uint64, stateDb typedb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, success func()) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
func (d *Downloader) LegacySync(id string, head entity.Hash, td, ttd *big.Int, mode SyncMode) error {
...
func (d *Downloader) synchronise(id string, hash entity.Hash, td, ttd *big.Int, mode SyncMode, beaconMode bool, beaconPing chan struct{}) error {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash entity.Hash, td, ttd *big.Int, beaconMode bool) (err error) {
	...
	defer func(start time.Time) {
		log.Debug("Synchronisation terminated", "elapsed", entity.PrettyDuration(time.Since(start)))
	}(time.Now())

	// Look up the sync boundaries: the entity ancestor and the target block
	var latest, pivot *block.Header
	if !beaconMode {
...
// a remote peer.
func (d *Downloader) fetchHead(p *peerConnection) (head *block.Header, pivot *block.Header, err error) {
...
func calculateRequestSpan(remoteHeight, localHeight uint64) (int64, int, int, uint64) {
	...
	// between us and the peer is 1-2 blocks, which is most entity
...
// findAncestor tries to locate the entity ancestor link of the local chain and
// a remote peers blockchain. In the general case when our node was in sync and
// on the correct chain, checking the top N links should already get us a match.
// In the rare scenario when we ended up on a long reorganisation (i.e. none of
// the head links match), we do a binary search to find the entity ancestor.
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *block.Header) (uint64, error) {
	...
	p.log.Debug("Looking for entity ancestor", "local", localHeight, "remote", remoteHeight)
	...
	// If the error returned does not reflect that a entity ancestor was not found, return it.
	// If the error reflects that a entity ancestor was not found, continue to binary search,
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode, remoteHeight, localHeight uint64, floor int64) (uint64, error) {
	...
	p.log.Trace("Span searching for entity ancestor", "count", count, "from", from, "skip", skip)
	headers, hashes, err := d.fetchHeadersByNumber(p, uint64(from), count, skip, false)
	if err != nil {
		return 0, err
	}
	// Wait for the remote response to the head fetch
	number, hash := uint64(0), entity.Hash{}
	...
	// Check if a entity ancestor was found
	...
	if hash != (entity.Hash{}) {
		...
		p.log.Debug("Found entity ancestor", "number", number, "hash", hash)
		return number, nil
	}
...
func (d *Downloader) findAncestorBinarySearch(p *peerConnection, mode SyncMode, remoteHeight uint64, floor int64) (uint64, error) {
	...
	p.log.Trace("Binary searching for entity ancestor", "start", start, "end", end)
	...
	p.log.Debug("Found entity ancestor", "number", start, "hash", hash)
	return start, nil
}
	...
	for {
		...
		if skeleton {
			...
			if n := len(headers); n > 0 {
				...
				// If the head is below the entity ancestor, we're actually deduplicating
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*block.Header) ([]*block.Header, []entity.Hash, int, error) {
...
func (d *Downloader) processHeaders(origin uint64, td, ttd *big.Int, beaconMode bool) error {
	...
	defer func() {
		if rollback > 0 {
			lastHeader, lastFastBlock, lastBlock := d.lightchain.CurrentHeader().Number, operationutils.Big0, operationutils.Big0
			if mode != LightSync {
			...
			curFastBlock, curBlock := operationutils.Big0, operationutils.Big0
			if mode != LightSync {
		...
		case task := <-d.headerProcCh:
			...
			if task == nil || len(task.headers) == 0 {
				...
				if !beaconMode {
					...
					// L: Sync begins, and finds entity ancestor at 11
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	blocks := make([]*block.Block, len(results))
	for i, result := range results {
		blocks[i] = block.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles)
	}
...
func (d *Downloader) commitSnapSyncData(results []*fetchResult, stateSync *stateSync) error {
	...
	blocks := make([]*block.Block, len(results))
	receipts := make([]block.Receipts, len(results))
	for i, result := range results {
		blocks[i] = block.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles)
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	b := block.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles)
	log.Debug("Committing snap sync pivot as new head", "number", b.Number(), "hash", b.Hash())

	// Commit the pivot block as the new head, will require full sync from here on
	if _, err := d.blockchain.InsertReceiptChain([]*block.Block{b}, []block.Receipts{result.Receipts}, d.ancientLimit); err != nil {
		return err
	}
	if err := d.blockchain.SnapSyncCommitHead(b.Hash()); err != nil {
		return err
	}
...
func (d *Downloader) readHeaderRange(last *block.Header, count int) []*block.Header {
	var (
		current = last
		headers []*block.Header
	)
