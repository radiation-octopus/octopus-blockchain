...
type fetchRequest struct {
	...
	Headers []*block.Header // Requested headers, sorted by request order
...
func newFetchResult(header *block.Header, fastSync bool) *fetchResult {
	item := &fetchResult{
		Header: header,
	}
	if !header.EmptyBody() {
...
type queue struct {
	...
	headerHead      entity.Hash                    // Hash of the last queued header to verify order
	headerTaskPool  map[uint64]*block.Header       // Pending header retrieval tasks, mapping starting indexes to skeleton headers
	...
	headerResults   []*block.Header                // Result cache accumulating the completed headers
	headerHashes    []entity.Hash                  // Result cache accumulating the completed header hashes
	...
	blockTaskPool  map[entity.Hash]*block.Header // Pending block (body) retrieval tasks, mapping hashes to headers
	...
	receiptTaskPool  map[entity.Hash]*block.Header // Pending receipt retrieval tasks, mapping hashes to headers
	...
	resultSize  utils.StorageSize // Approximate size of a block (exponential moving average)
...
func (q *queue) Reset(blockCacheLimit int, thresholdInitialSize int) {
	...
	q.headerHead = entity.Hash{}
	q.headerPendPool = make(map[string]*fetchRequest)

	q.blockTaskPool = make(map[entity.Hash]*block.Header)
	q.blockTaskQueue.Reset()
	q.blockPendPool = make(map[string]*fetchRequest)

	q.receiptTaskPool = make(map[entity.Hash]*block.Header)
...
func (q *queue) ScheduleSkeleton(from uint64, skeleton []*block.Header) {
	...
	q.headerHashes = make([]entity.Hash, len(skeleton)*MaxHeaderFetch)
	q.headerProced = 0
	q.headerOffset = from
...
// skeleton.
func (q *queue) RetrieveHeaders() ([]*block.Header, []entity.Hash, int) {
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *queue) Schedule(headers []*block.Header, hashes []entity.Hash, from uint64) []*block.Header {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	for i, header := range headers {
		...
		if q.headerHead != (entity.Hash{}) && q.headerHead != header.ParentHash {
...
func (q *queue) Results(block bool) []*fetchResult {
	...
	for _, result := range results {
		...
		q.resultSize = utils.StorageSize(blockCacheSizeWeight)*size +
			(1-utils.StorageSize(blockCacheSizeWeight))*q.resultSize
	}
	// Using the newly calibrated resultsize, figure out the new throttle limit
	// on the result cache
	throttleThreshold := uint64((utils.StorageSize(blockCacheMemory) + q.resultSize - 1) / q.resultSize)
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[entity.Hash]*block.Header, taskQueue *prque.Prque,
...
func (q *queue) DeliverHeaders(id string, headers []*block.Header, hashes []entity.Hash, headerProcCh chan *headerTask) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	if ready > 0 {
		...
		processHeaders := make([]*block.Header, ready)
		copy(processHeaders, q.headerResults[q.headerProced:q.headerProced+ready])

		processHashes := make([]entity.Hash, ready)
...
func (q *queue) DeliverBodies(id string, txLists [][]*block.Transaction, txListHashes []entity.Hash, uncleLists [][]*block.Header, uncleListHashes []entity.Hash) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()

	validate := func(index int, header *block.Header) error {
...
func (q *queue) DeliverReceipts(id string, receiptList [][]*block.Receipt, receiptListHashes []entity.Hash) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
...
// to access the queue, so they already need a lock anyway.
func (q *queue) deliver(id string, taskPool map[entity.Hash]*block.Header,
	...
	results int, validate func(index int, header *block.Header) error,
