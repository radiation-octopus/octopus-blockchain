...
type NewBlockHashesPacket []struct {
	Hash   entity.Hash // Hash of one particular block being announced
...
func (p *NewBlockHashesPacket) Unpack() ([]entity.Hash, []uint64) {
	var (
		hashes  = make([]entity.Hash, len(*p))
...
type TransactionsPacket []*block.Transaction
...
type HashOrNumber struct {
	Hash   entity.Hash // Block hash from which to retrieve headers (excludes Number)
...
type BlockHeadersPacket []*block.Header
...
type GetBlockBodiesPacket []entity.Hash
...
type BlockBody struct {
	Transactions []*block.Transaction // Transactions contained within a block
	Uncles       []*block.Header      // Uncles contained within a block
}

// Unpack retrieves the transactions and uncles from the range packet and returns
// them in a split flat format that's more consistent with the internal data structures.
func (p *BlockBodiesPacket) Unpack() ([][]*block.Transaction, [][]*block.Header) {
	var (
		txset    = make([][]*block.Transaction, len(*p))
		uncleset = make([][]*block.Header, len(*p))
	)
	for i, body := range *p {
...
type GetNodeDataPacket []entity.Hash
...
type GetReceiptsPacket []entity.Hash
...
type ReceiptsPacket [][]*block.Receipt
...
type NewPooledTransactionHashesPacket []entity.Hash

// GetPooledTransactionsPacket represents a transaction query.
type GetPooledTransactionsPacket []entity.Hash
...
type PooledTransactionsPacket []*block.Transaction
