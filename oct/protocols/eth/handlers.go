...
func ServiceGetBlockHeadersQuery(chain *blockchain.BlockChain, query *GetBlockHeadersPacket, peer *Peer) []rlp.RawValue {
	if query.Skip == 0 {
...
func serviceNonContiguousBlockHeaderQuery(chain *blockchain.BlockChain, query *GetBlockHeadersPacket, peer *Peer) []rlp.RawValue {
	hashMode := query.Origin.Hash != (entity.Hash{})
	first := true
	...
	for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit &&
		...
		if rlpData, err := rlp.EncodeToBytes(origin); err != nil {
			...
			bytes += utils.StorageSize(len(rlpData))
		}
		...
		case hashMode && query.Reverse:
			...
			if ancestor == 0 {
				...
				unknown = (query.Origin.Hash == entity.Hash{})
			}
		case hashMode && !query.Reverse:
...
func serviceContiguousBlockHeaderQuery(chain *blockchain.BlockChain, query *GetBlockHeadersPacket) []rlp.RawValue {
	count := query.Amount
	if count > maxHeadersServe {
		count = maxHeadersServe
	}
	if query.Origin.Hash == (entity.Hash{}) {
...
// exposed to allow external packages to test protocol behavior.
func ServiceGetBlockBodiesQuery(chain *blockchain.BlockChain, query GetBlockBodiesPacket) []rlp.RawValue {
...
// exposed to allow external packages to test protocol behavior.
func ServiceGetNodeDataQuery(chain *blockchain.BlockChain, query GetNodeDataPacket) [][]byte {
...
// exposed to allow external packages to test protocol behavior.
func ServiceGetReceiptsQuery(chain *blockchain.BlockChain, query GetReceiptsPacket) []rlp.RawValue {
	// Gather state data until the fetch or network limits is reached
	var (
		bytes    int
	...
	for lookups, hash := range query {
		...
		if results == nil {
			if header := chain.GetHeaderByHash(hash); header == nil || header.ReceiptHash != block.EmptyRootHash {
				continue
			}
		}
...
func handleNewBlock(backend Backend, msg Decoder, peer *Peer) error {
	...
	if hash := block.CalcUncleHash(ann.Block.Uncles()); hash != ann.Block.UncleHash() {
		log.Warn("Propagated block has invalid uncles", "have", hash, "exp", ann.Block.UncleHash())
		return nil // TODO(karalabe): return error eventually, but wait a few releases
	}
	if hash := block.DeriveSha(ann.Block.Transactions(), trie.NewStackTrie(nil)); hash != ann.Block.TxHash() {
...
func handleBlockHeaders66(backend Backend, msg Decoder, peer *Peer) error {
	...
	metadata := func() interface{} {
		hashes := make([]entity.Hash, len(res.BlockHeadersPacket))
...
func handleBlockBodies66(backend Backend, msg Decoder, peer *Peer) error {
	...
	metadata := func() interface{} {
		var (
			txsHashes   = make([]entity.Hash, len(res.BlockBodiesPacket))
			uncleHashes = make([]entity.Hash, len(res.BlockBodiesPacket))
		)
		...
		return [][]entity.Hash{txsHashes, uncleHashes}
	}
	return peer.dispatchResponse(&Response{
		id:   res.RequestId,
		code: BlockBodiesMsg,
...
func handleReceipts66(backend Backend, msg Decoder, peer *Peer) error {
	...
	metadata := func() interface{} {
		hasher := trie.NewStackTrie(nil)
		hashes := make([]entity.Hash, len(res.ReceiptsPacket))
...
func answerGetPooledTransactions(backend Backend, query GetPooledTransactionsPacket, peer *Peer) ([]entity.Hash, []rlp.RawValue) {
