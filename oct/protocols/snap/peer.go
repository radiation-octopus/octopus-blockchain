...
func (p *Peer) RequestAccountRange(id uint64, root entity.Hash, origin, limit entity.Hash, bytes uint64) error {
	p.logger.Trace("Fetching range of accounts", "reqid", id, "root", root, "origin", origin, "limit", limit, "bytes", utils.StorageSize(bytes))
...
func (p *Peer) RequestStorageRanges(id uint64, root entity.Hash, accounts []entity.Hash, origin, limit []byte, bytes uint64) error {
	if len(accounts) == 1 && origin != nil {
		p.logger.Trace("Fetching range of large storage slots", "reqid", id, "root", root, "account", accounts[0], "origin", entity.BytesToHash(origin), "limit", entity.BytesToHash(limit), "bytes", utils.StorageSize(bytes))
	} else {
		p.logger.Trace("Fetching ranges of small storage slots", "reqid", id, "root", root, "accounts", len(accounts), "first", accounts[0], "bytes", utils.StorageSize(bytes))
	}
...
func (p *Peer) RequestByteCodes(id uint64, hashes []entity.Hash, bytes uint64) error {
	p.logger.Trace("Fetching set of byte codes", "reqid", id, "hashes", len(hashes), "bytes", utils.StorageSize(bytes))
...
func (p *Peer) RequestTrieNodes(id uint64, root entity.Hash, paths []TrieNodePathSet, bytes uint64) error {
	p.logger.Trace("Fetching set of trie nodes", "reqid", id, "root", root, "pathsets", len(paths), "bytes", utils.StorageSize(bytes))
