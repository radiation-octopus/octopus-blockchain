...
type GetAccountRangePacket struct {
	...
	Root   entity.Hash // Root hash of the account trie to serve
	Origin entity.Hash // Hash of the first account to retrieve
	Limit  entity.Hash // Hash of the last account to retrieve
...
type AccountData struct {
	Hash entity.Hash  // Hash of the account
...
func (p *AccountRangePacket) Unpack() ([]entity.Hash, [][]byte, error) {
	var (
		hashes   = make([]entity.Hash, len(p.Accounts))
...
type GetStorageRangesPacket struct {
	ID       uint64        // Request ID to match up responses with
	Root     entity.Hash   // Root hash of the account trie to serve
	Accounts []entity.Hash // Account hashes of the storage tries to serve
...
type StorageData struct {
	Hash entity.Hash // Hash of the storage slot
...
func (p *StorageRangesPacket) Unpack() ([][]entity.Hash, [][][]byte) {
	var (
		hashset = make([][]entity.Hash, len(p.Slots))
		slotset = make([][][]byte, len(p.Slots))
	)
	for i, slots := range p.Slots {
		hashset[i] = make([]entity.Hash, len(slots))
...
type GetByteCodesPacket struct {
	ID     uint64        // Request ID to match up responses with
	Hashes []entity.Hash // Code hashes to retrieve the code for
	Bytes  uint64        // Soft limit at which to stop returning data
}
