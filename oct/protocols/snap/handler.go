...
func ServiceGetAccountRangeQuery(chain *blockchain.BlockChain, req *GetAccountRangePacket) ([]*AccountData, [][]byte) {
	if req.Bytes > softResponseLimit {
		req.Bytes = softResponseLimit
	}
	// Retrieve the requested state and bail out if non existent
	//tr, err := trie.New(entity.Hash{}, req.Root, chain.StateCache().TrieDB())
	//if err != nil {
	//	return nil, nil
	//}
	...
	//)
	//for it.Next() {
	//	hash, account := it.Hash(), entity.CopyBytes(it.Account())
	//
	//	// Track the returned interval for the Merkle proofs
	//	last = hash
	//
	//	// Assemble the reply item
	//	size += uint64(entity.HashLength + len(account))
...
func ServiceGetStorageRangesQuery(chain *blockchain.BlockChain, req *GetStorageRangesPacket) ([][]*StorageData, [][]byte) {
	if req.Bytes > softResponseLimit {
		req.Bytes = softResponseLimit
	}
	...
	//	var origin entity.Hash
	//	if len(req.Origin) > 0 {
	//		origin, req.Origin = entity.BytesToHash(req.Origin), nil
	//	}
	//	var limit = entity.HexToHash("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")
	//	if len(req.Limit) > 0 {
	//		limit, req.Limit = entity.BytesToHash(req.Limit), nil
	//	}
	//	// Retrieve the requested state and bail out if non existent
	...
	//			break
	//		}
	//		hash, slot := it.Hash(), entity.CopyBytes(it.Slot())
	//
	//		// Track the returned interval for the Merkle proofs
	//		last = hash
	//
	//		// Assemble the reply item
	//		size += uint64(entity.HashLength + len(slot))
	...
	//	if origin != (entity.Hash{}) || (abort && len(storage) > 0) {
	//		// Request started at a non-zero hash or was capped prematurely, add
	//		// the endpoint Merkle proofs
	//		accTrie, err := trie.New(entity.Hash{}, req.Root, chain.StateCache().TrieDB())
	//		if err != nil {
	//			return nil, nil
	//		}
...
// It is exposed to allow external packages to test protocol behavior.
func ServiceGetByteCodesQuery(chain *blockchain.BlockChain, req *GetByteCodesPacket) [][]byte {
	if req.Bytes > softResponseLimit {
		req.Bytes = softResponseLimit
	}
...
// It is exposed to allow external packages to test protocol behavior.
//func ServiceGetTrieNodesQuery(chain *blockchain.BlockChain, req *GetTrieNodesPacket, start time.Time) ([][]byte, error) {
//	if req.Bytes > softResponseLimit {
//		req.Bytes = softResponseLimit
//	}
...
//
//	accTrie, err := trie.NewSecure(entity.Hash{}, req.Root, triedb)
//	if err != nil {
...
//			account, err := snap.Account(entity.BytesToHash(pathset[0]))
//			loads++ // always account database reads, even for failures
//			if err != nil || account == nil {
//				break
//			}
//			stTrie, err := trie.NewSecure(entity.BytesToHash(pathset[0]), entity.BytesToHash(account.Root), triedb)
//			loads++ // always account database reads, even for failures
//			if err != nil {
//				break
//			}
