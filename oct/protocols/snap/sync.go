...
import (
	...
	"github.com/radiation-octopus/octopus-blockchain/p2p/msgrate"
...
var (
	...
	emptyRoot = entity.HexToHash("56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421")
...
type accountRequest struct {
	...
	origin entity.Hash // First account requested to allow continuation checks
	limit  entity.Hash // Last account requested to allow non-overlapping chunking
...
type accountResponse struct {
	...
	hashes   []entity.Hash          // Account hashes in the returned range
	accounts []*entity.StateAccount // Expanded accounts in the returned range
...
type bytecodeRequest struct {
	...
	hashes []entity.Hash // Bytecode hashes to validate responses
	task   *accountTask  // Task which this request is filling (only access fields through the runloop!!)
}
...
type bytecodeResponse struct {
	...
	hashes []entity.Hash // Hashes of the bytecode to avoid double hashing
...
type storageRequest struct {
	...
	accounts []entity.Hash // Account hashes to validate responses
	roots    []entity.Hash // Storage roots to validate responses

	origin entity.Hash // First storage slot requested to allow continuation checks
	limit  entity.Hash // Last storage slot requested to allow non-overlapping chunking
...
type storageResponse struct {
	...
	accounts []entity.Hash // Account hashes requested, may be only partially filled
	roots    []entity.Hash // Storage roots requested, may be only partially filled

	hashes [][]entity.Hash // Storage slot hashes in the returned range
...
type trienodeHealRequest struct {
	...
	hashes []entity.Hash   // Trie node hashes to validate responses
...
type trienodeHealResponse struct {
	...
	hashes []entity.Hash   // Hashes of the trie nodes to avoid double hashing
...
type accountTask struct {
	...
	Next     entity.Hash                    // Next account to sync in this interval
	Last     entity.Hash                    // Last account to sync in this interval
	SubTasks map[entity.Hash][]*storageTask // Storage intervals needing fetching for large contracts
	...
	codeTasks  map[entity.Hash]struct{}    // Code hashes that need retrieval
	stateTasks map[entity.Hash]entity.Hash // Account hashes->roots that need full state retrieval

	genBatch typedb.Batch    // Batch used by the node generator
...
type storageTask struct {
	...
	// These fields are internals used during runtime
	root entity.Hash     // Storage root hash for this instance
...
type healTask struct {
	...
	trieTasks map[entity.Hash]trie.SyncPath // Set of trie node tasks currently queued for retrieval
	codeTasks map[entity.Hash]struct{}      // Set of byte code tasks currently queued for retrieval
}
...
type SyncProgress struct {
	...
	AccountBytes   utils.StorageSize // Number of account trie bytes persisted to disk
	BytecodeSynced uint64            // Number of bytecodes downloaded
	BytecodeBytes  utils.StorageSize // Number of bytecode bytes downloaded
	StorageSynced  uint64            // Number of storage slots downloaded
	StorageBytes   utils.StorageSize // Number of storage trie bytes persisted to disk

	// Status report during healing phase
	TrienodeHealSynced uint64            // Number of state trie nodes downloaded
	TrienodeHealBytes  utils.StorageSize // Number of state trie bytes persisted to disk
	BytecodeHealSynced uint64            // Number of bytecodes downloaded
	BytecodeHealBytes  utils.StorageSize // Number of bytecodes persisted to disk
}
...
type SyncPeer interface {
	...
	RequestAccountRange(id uint64, root, origin, limit entity.Hash, bytes uint64) error

	// RequestStorageRanges fetches a batch of storage slots belonging to one or
	// more accounts. If slots from only one accout is requested, an origin marker
	// may also be used to retrieve from there.
	RequestStorageRanges(id uint64, root entity.Hash, accounts []entity.Hash, origin, limit []byte, bytes uint64) error

	// RequestByteCodes fetches a batch of bytecodes by hash.
	RequestByteCodes(id uint64, hashes []entity.Hash, bytes uint64) error

	// RequestTrieNodes fetches a batch of account or storage trie nodes rooted in
	// a specificstate trie.
	RequestTrieNodes(id uint64, root entity.Hash, paths []TrieNodePathSet, bytes uint64) error
...
type Syncer struct {
	db typedb.KeyValueStore // Database to store the trie nodes into (and dedup)

	root    entity.Hash    // Current state trie root being synced
	...
	stateWriter        typedb.Batch      // Shared batch writer used for persisting raw states
	accountHealed      uint64            // Number of accounts downloaded during the healing stage
	accountHealedBytes utils.StorageSize // Number of raw account bytes persisted to disk during the healing stage
	storageHealed      uint64            // Number of storage slots downloaded during the healing stage
	storageHealedBytes utils.StorageSize // Number of raw storage bytes persisted to disk during the healing stage
...
// snap protocol.
func NewSyncer(db typedb.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
...
func (s *Syncer) Sync(root entity.Hash, cancel chan struct{}) error {
	...
	s.root = root
	s.healer = &healTask{
		scheduler: operationdb.NewStateSync(root, s.db, s.onHealState),
		trieTasks: make(map[entity.Hash]trie.SyncPath),
		codeTasks: make(map[entity.Hash]struct{}),
	}
...
func (s *Syncer) loadSyncStatus() {
	...
	//		for _, task := range s.tasks {
	//			task.genBatch = typedb.HookedBatch{
	//				Batch: s.db.NewBatch(),
	//				OnPut: func(key []byte, value []byte) {
	//					s.accountBytes += entity.StorageSize(len(key) + len(value))
	//				},
	//			}
	...
	//					subtask.genBatch = typedb.HookedBatch{
	//						Batch: s.db.NewBatch(),
	//						OnPut: func(key []byte, value []byte) {
	//							s.storageBytes += entity.StorageSize(len(key) + len(value))
	//						},
	//					}
	...
	var next entity.Hash
	step := new(big.Int).Sub(
		new(big.Int).Div(
			new(big.Int).Exp(operationutils.Big2, operationutils.Big256, nil),
			big.NewInt(int64(accountConcurrency)),
		), operationutils.Big1,
	)
	for i := 0; i < accountConcurrency; i++ {
		last := entity.BigToHash(new(big.Int).Add(next.Big(), step))
		if i == accountConcurrency-1 {
			// Make sure we don't overflow if the step is not a proper divisor
			last = entity.HexToHash("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")
		}
		batch := typedb.HookedBatch{
			Batch: s.db.NewBatch(),
			OnPut: func(key []byte, value []byte) {
				s.accountBytes += utils.StorageSize(len(key) + len(value))
			},
		}
		s.tasks = append(s.tasks, &accountTask{
			Next:     next,
			Last:     last,
			SubTasks: make(map[entity.Hash][]*storageTask),
			genBatch: batch,
			genTrie:  trie.NewStackTrie(batch),
		})
		log.Debug("Created account sync task", "from", next, "last", last)
		next = entity.BigToHash(new(big.Int).Add(last.Big(), operationutils.Big1))
	}
}
...
func (s *Syncer) assignStorageTasks(success chan *storageResponse, fail chan *storageRequest, cancel chan struct{}) {
	...
	for _, task := range s.tasks {
		...
		var (
			accounts = make([]entity.Hash, 0, storageSets)
			roots    = make([]entity.Hash, 0, storageSets)
			subtask  *storageTask
		)
		for account, subtasks := range task.SubTasks {
...
func (s *Syncer) processAccountResponse(res *accountResponse) {
	...
	res.task.codeTasks = make(map[entity.Hash]struct{})
	res.task.stateTasks = make(map[entity.Hash]entity.Hash)

	resumed := make(map[entity.Hash]struct{})

	res.task.pend = 0
	for i, account := range res.accounts {
		// Check if the account is a contract with an unknown code
		if !bytes.Equal(account.CodeHash, emptyCode[:]) {
			if !rawdb.HasCodeWithPrefix(s.db, entity.BytesToHash(account.CodeHash)) {
				res.task.codeTasks[entity.BytesToHash(account.CodeHash)] = struct{}{}
		...
		if account.Root != emptyRoot {
			if ok, err := s.db.IsHas(account.Root[:]); err != nil || !ok {
...
func (s *Syncer) processBytecodeResponse(res *bytecodeResponse) {
	...
	for i, hash := range res.hashes {
		...
		for j, account := range res.task.res.accounts {
			if res.task.needCode[j] && hash == entity.BytesToHash(account.CodeHash) {
	...
	bytes := utils.StorageSize(batch.ValueSize())
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	batch := typedb.HookedBatch{
		Batch: s.db.NewBatch(),
		OnPut: func(key []byte, value []byte) {
			s.storageBytes += utils.StorageSize(len(key) + len(value))
		},
	}
	var (
		slots           int
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > typedb.IdealBatchSize || res.subTask.done {
...
func (s *Syncer) processTrienodeHealResponse(res *trienodeHealResponse) {
	for i, hash := range res.hashes {
		...
		s.trienodeHealBytes += utils.StorageSize(len(node))
	...
	log.Debug("Persisted set of healing data", "type", "trienodes", "bytes", utils.StorageSize(batch.ValueSize()))
}
...
func (s *Syncer) processBytecodeHealResponse(res *bytecodeHealResponse) {
	for i, hash := range res.hashes {
		...
		s.bytecodeHealBytes += utils.StorageSize(len(node))
	...
	log.Debug("Persisted set of healing data", "type", "bytecode", "bytes", utils.StorageSize(batch.ValueSize()))
}
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	if task.genBatch.ValueSize() > typedb.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []entity.Hash, accounts [][]byte, proof [][]byte) error {
	size := utils.StorageSize(len(hashes) * entity.HashLength)
	for _, account := range accounts {
		size += utils.StorageSize(len(account))
	}
	for _, node := range proof {
		size += utils.StorageSize(len(node))
	}
	...
	//	return err
	//}
	accs := make([]*entity.StateAccount, len(accounts))
	for i, account := range accounts {
		acc := new(entity.StateAccount)
...
func (s *Syncer) onByteCodes(peer SyncPeer, id uint64, bytecodes [][]byte) error {
	var size utils.StorageSize
	for _, code := range bytecodes {
		size += utils.StorageSize(len(code))
	}
	logger := peer.Log().New("reqid", id)
...
func (s *Syncer) OnStorage(peer SyncPeer, id uint64, hashes [][]entity.Hash, slots [][][]byte, proof [][]byte) error {
	...
	for _, hashset := range hashes {
		size += utils.StorageSize(entity.HashLength * len(hashset))
		hashCount += len(hashset)
	}
	for _, slotset := range slots {
		for _, slot := range slotset {
			size += utils.StorageSize(len(slot))
		}
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	if len(paths) == 1 {
		...
		//rawdb.WriteAccountSnapshot(s.stateWriter, entity.BytesToHash(paths[0]), blob)
		s.accountHealed += 1
		//s.accountHealedBytes += utils.StorageSize(1 + entity.HashLength + len(blob))
	}
	if len(paths) == 2 {
		//rawdb.WriteStorageSnapshot(s.stateWriter, entity.BytesToHash(paths[0]), entity.BytesToHash(paths[1]), value)
		s.storageHealed += 1
		s.storageHealedBytes += utils.StorageSize(1 + 2*entity.HashLength + len(value))
	}
	if s.stateWriter.ValueSize() > typedb.IdealBatchSize {
...
var hashSpace = new(big.Int).Exp(operationutils.Big2, operationutils.Big256, nil)
...
func (s *Syncer) reportSyncProgress(force bool) {
	...
	log.Info("State sync in progress", "synced", progress, "state", synced,
		"accounts", accounts, "slots", storage, "codes", bytecode, "eta", entity.PrettyDuration(estTime-elapsed))
}
...
func estimateRemainingSlots(hashes int, last entity.Hash) (uint64, error) {
	if last == (entity.Hash{}) {
...
func sortByAccountPath(hashes []entity.Hash, paths []trie.SyncPath) ([]entity.Hash, []trie.SyncPath, []TrieNodePathSet) {
