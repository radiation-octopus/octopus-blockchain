...
type blockFees struct {
	...
	blockNumber uint64
	header      *block.Header
	block       *block.Block // only set if reward percentiles are requested
...
func (oracle *Oracle) resolveBlockRange(ctx context.Context, lastBlock rpc.BlockNumber, blocks int) (*block.Block, []*block.Receipt, uint64, int, error) {
	var (
...
func (oracle *Oracle) FeeHistory(ctx context.Context, blocks int, unresolvedLastBlock rpc.BlockNumber, rewardPercentiles []float64) (*big.Int, [][]*big.Int, []*big.Int, []float64, error) {
	if blocks < 1 {
		return operationutils.Big0, nil, nil, nil, nil // returning with no data and no error means there are no retrievable blocks
	}
	...
	for i, p := range rewardPercentiles {
		if p < 0 || p > 100 {
			return operationutils.Big0, nil, nil, nil, fmt.Errorf("%w: %f", errInvalidPercentile, p)
		}
		if i > 0 && p < rewardPercentiles[i-1] {
			return operationutils.Big0, nil, nil, nil, fmt.Errorf("%w: #%d:%f > #%d:%f", errInvalidPercentile, i-1, rewardPercentiles[i-1], i, p)
		}
	}
	var (
		pendingBlock    *block.Block
	...
	if err != nil || blocks == 0 {
		return operationutils.Big0, nil, nil, nil, err
	}
	...
	for ; blocks > 0; blocks-- {
		fees := <-results
		if fees.err != nil {
			return operationutils.Big0, nil, nil, nil, fees.err
		}
	...
	if firstMissing == 0 {
		return operationutils.Big0, nil, nil, nil, nil
	}
	if len(rewardPercentiles) != 0 {
