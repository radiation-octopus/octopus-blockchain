...
var (
	DefaultMaxPrice    = big.NewInt(500 * entity.Gcao)
	DefaultIgnorePrice = big.NewInt(2 * entity.Cao)
)
...
func NewOracle(backend OracleBackend, params Config) *Oracle {
	...
	headEvent := make(chan event.ChainHeadEvent, 1)
	backend.SubscribeChainHeadEvent(headEvent)
	go func() {
		var lastHead entity.Hash
...
func (oracle *Oracle) SuggestTipCap(ctx context.Context) (*big.Int, error) {
	...
	for sent < oracle.checkBlocks && number > 0 {
		go oracle.getBlockValues(ctx, block.MakeSigner(big.NewInt(int64(number))), number, sampleNumber, oracle.ignorePrice, result, quit)
		sent++
		exp++
		number--
	}
...
func newSorter(txs []*block.Transaction, baseFee *big.Int) *txSorter {
	return &txSorter{
		txs:     txs,
		baseFee: baseFee,
	}
...
func (oracle *Oracle) getBlockValues(ctx context.Context, signer block.Signer, blockNum uint64, limit int, ignoreUnder *big.Int, result chan results, quit chan struct{}) {
	b, err := oracle.backend.BlockByNumber(ctx, rpc.BlockNumber(blockNum))
	if b == nil {
		select {
	...
	txs := make([]*block.Transaction, len(b.Transactions()))
	copy(txs, b.Transactions())
	sorter := newSorter(txs, b.BaseFee())
	sort.Sort(sorter)

	var prices []*big.Int
	for _, tx := range sorter.txs {
		tip, _ := tx.EffectiveGasTip(b.BaseFee())
		if ignoreUnder != nil && tip.Cmp(ignoreUnder) == -1 {
			continue
		}
		sender, err := block.Sender(signer, tx)
		if err == nil && sender != b.Coinbase() {
