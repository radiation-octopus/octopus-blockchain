...
type txAnnounce struct {
	...
	hashes []entity.Hash // Batch of transaction hashes being announced
}

// txRequest represents an in-flight transaction retrieval request destined to
// a specific peers.
type txRequest struct {
	hashes []entity.Hash            // Transactions having been requested
	stolen map[entity.Hash]struct{} // Deliveries by someone else (don't re-request)
...
type txDelivery struct {
	origin string        // Identifier of the peer originating the notification
	hashes []entity.Hash // Batch of transaction hashes having been delivered
...
type TxFetcher struct {
	...
	waitlist  map[entity.Hash]map[string]struct{} // Transactions waiting for an potential broadcast
	waittime  map[entity.Hash]mclock.AbsTime      // Timestamps when transactions were added to the waitlist
	waitslots map[string]map[entity.Hash]struct{} // Waiting announcement sgroupped by peer (DoS protection)

	// Stage 2: Queue of transactions that waiting to be allocated to some peer
	// to be retrieved directly.
	announces map[string]map[entity.Hash]struct{} // Set of announced transactions, grouped by origin peer
	announced map[entity.Hash]map[string]struct{} // Set of download locations, grouped by transaction hash

	// Stage 3: Set of transactions currently being retrieved, some which may be
	// fulfilled and some rescheduled. Note, this step shares 'announces' from the
	// previous stage to avoid having to duplicate (need it for DoS checks).
	fetching   map[entity.Hash]string              // Transaction set currently being retrieved
	requests   map[string]*txRequest               // In-flight transaction retrievals
	alternates map[entity.Hash]map[string]struct{} // In-flight transaction alternate origins if retrieval fails

	// Callbacks
	hasTx    func(entity.Hash) bool             // Retrieves a tx from the local txpool
	addTxs   func([]*block.Transaction) []error // Insert a batch of transactions into local txpool
	fetchTxs func(string, []entity.Hash) error  // Retrieves a set of txs from a remote peer
...
func NewTxFetcher(hasTx func(entity.Hash) bool, addTxs func([]*block.Transaction) []error, fetchTxs func(string, []entity.Hash) error) *TxFetcher {
...
func NewTxFetcherForTests(
	hasTx func(entity.Hash) bool, addTxs func([]*block.Transaction) []error, fetchTxs func(string, []entity.Hash) error,
	...
	return &TxFetcher{
		...
		waitlist:    make(map[entity.Hash]map[string]struct{}),
		waittime:    make(map[entity.Hash]mclock.AbsTime),
		waitslots:   make(map[string]map[entity.Hash]struct{}),
		announces:   make(map[string]map[entity.Hash]struct{}),
		announced:   make(map[entity.Hash]map[string]struct{}),
		fetching:    make(map[entity.Hash]string),
		requests:    make(map[string]*txRequest),
		alternates:  make(map[entity.Hash]map[string]struct{}),
...
func (f *TxFetcher) Notify(peer string, hashes []entity.Hash) error {
	...
	var (
		unknowns               = make([]entity.Hash, 0, len(hashes))
...
func (f *TxFetcher) Enqueue(peer string, txs []*block.Transaction, direct bool) error {
	...
	var (
		added       = make([]entity.Hash, 0, len(txs))
		duplicate   int64
		underpriced int64
		otherreject int64
	)
	...
	for i, err := range errs {
		...
		// announcement.
		if errors.Is(err, blockchain.ErrUnderpriced) || errors.Is(err, blockchain.ErrReplaceUnderpriced) {
			for f.underpriced.Cardinality() >= maxTxUnderpricedSetSize {
				f.underpriced.Pop()
			}
			f.underpriced.Add(txs[i].Hash())
		}
		// Track a few interesting failure block
		switch {
		case err == nil: // Noop, but need to handle to not count these

		case errors.Is(err, blockchain.ErrAlreadyKnown):
			duplicate++

		case errors.Is(err, blockchain.ErrUnderpriced) || errors.Is(err, blockchain.ErrReplaceUnderpriced):
			underpriced++
	...
	for {
			...
			for _, hash := range ann.hashes {
				...
				if f.alternates[hash] != nil {
					...
					if announces := f.announces[ann.origin]; announces != nil {
						...
						f.announces[ann.origin] = map[entity.Hash]struct{}{hash: {}}
					}
					continue
				}
				...
				if f.waitlist[hash] != nil {
					...
					if waitslots := f.waitslots[ann.origin]; waitslots != nil {
						...
						f.waitslots[ann.origin] = map[entity.Hash]struct{}{hash: {}}
					}
					continue
				}
				...
				if time.Duration(f.clock.Now()-instance)+txGatherSlack > txArriveTimeout {
					...
					for peer := range f.waitlist[hash] {
						if announces := f.announces[peer]; announces != nil {
							...
							f.announces[peer] = map[entity.Hash]struct{}{hash: {}}
						}
		...
		case delivery := <-f.cleanup:
			...
			for _, hash := range delivery.hashes {
				if _, ok := f.waitlist[hash]; ok {
					...
					if origin, ok := f.fetching[hash]; ok && (origin != delivery.origin || !delivery.direct) {
						...
						if stolen == nil {
							f.requests[origin].stolen = make(map[entity.Hash]struct{})
							stolen = f.requests[origin].stolen
						}
						stolen[hash] = struct{}{}
					}
			...
			if delivery.direct {
				...
				delivered := make(map[entity.Hash]struct{})
				for _, hash := range delivery.hashes {
...
func (f *TxFetcher) scheduleFetches(timer *mclock.Timer, timeout chan struct{}, whitelist map[string]struct{}) {
	...
	f.forEachPeer(actives, func(peer string) {
		...
		hashes := make([]entity.Hash, 0, maxTxRetrievals)
		f.forEachHash(f.announces[peer], func(hash entity.Hash) bool {
...
// testing it does a deterministic sorted random to allow reproducing issues.
func (f *TxFetcher) forEachHash(hashes map[entity.Hash]struct{}, do func(hash entity.Hash) bool) {
	// If we're running production, use whatever Go's map gives us
	if f.rand == nil {
	...
	// We're running the test suite, make iteration deterministic
	list := make([]entity.Hash, 0, len(hashes))
	for hash := range hashes {
...
func sortHashes(slice []entity.Hash) {
...
// used in tests to simulate random map iteration but keep it deterministic.
func rotateHashes(slice []entity.Hash, n int) {
	orig := make([]entity.Hash, len(slice))
	copy(orig, slice)
