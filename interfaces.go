...
type ChainReader interface {
	...
	BlockByNumber(ctx context.Context, number *big.Int) (*block.Block, error)
	HeaderByHash(ctx context.Context, hash entity.Hash) (*block.Header, error)
	HeaderByNumber(ctx context.Context, number *big.Int) (*block.Header, error)
	TransactionCount(ctx context.Context, blockHash entity.Hash) (uint, error)
	TransactionInBlock(ctx context.Context, blockHash entity.Hash, index uint) (*block.Transaction, error)
...
type TransactionReader interface {
	...
	// it's not pending.
	TransactionByHash(ctx context.Context, txHash entity.Hash) (tx *block.Transaction, isPending bool, err error)
	// TransactionReceipt returns the receipt of a mined transaction. Note that the
	// transaction may not be included in the current canonical chain even if a receipt
	// exists.
	TransactionReceipt(ctx context.Context, txHash entity.Hash) (*block.Receipt, error)
}
...
type ChainStateReader interface {
	BalanceAt(ctx context.Context, account entity.Address, blockNumber *big.Int) (*big.Int, error)
	StorageAt(ctx context.Context, account entity.Address, key entity.Hash, blockNumber *big.Int) ([]byte, error)
	CodeAt(ctx context.Context, account entity.Address, blockNumber *big.Int) ([]byte, error)
	NonceAt(ctx context.Context, account entity.Address, blockNumber *big.Int) (uint64, error)
}
...
type CallMsg struct {
	From      entity.Address  // the sender of the 'transaction'
	To        *entity.Address // the destination contract (nil for contract creation)
...
type FilterQuery struct {
	BlockHash *entity.Hash     // used by eth_getLogs, return logs only from block with this hash
	FromBlock *big.Int         // beginning of the queried range, nil means genesis block
	ToBlock   *big.Int         // end of the range, nil means latest block
	Addresses []entity.Address // restricts matches to events created by specific contracts
...
type TransactionSender interface {
	SendTransaction(ctx context.Context, tx *block.Transaction) error
}
...
// entityly used to display the result of ’unconfirmed’ actions (e.g. wallet value
...
type PendingStateReader interface {
	PendingBalanceAt(ctx context.Context, account entity.Address) (*big.Int, error)
	PendingStorageAt(ctx context.Context, account entity.Address, key entity.Hash) ([]byte, error)
	PendingCodeAt(ctx context.Context, account entity.Address) ([]byte, error)
	PendingNonceAt(ctx context.Context, account entity.Address) (uint64, error)
...
type PendingStateEventer interface {
	SubscribePendingTransactions(ctx context.Context, ch chan<- *block.Transaction) (Subscription, error)
}
