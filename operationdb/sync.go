...
func NewStateSync(root entity.Hash, database typedb.KeyValueReader, onLeaf func(paths [][]byte, leaf []byte) error) *trie.Sync {
	...
	var onSlot func(paths [][]byte, hexpath []byte, leaf []byte, parent entity.Hash) error
	if onLeaf != nil {
		onSlot = func(paths [][]byte, hexpath []byte, leaf []byte, parent entity.Hash) error {
	...
	onAccount := func(paths [][]byte, hexpath []byte, leaf []byte, parent entity.Hash) error {
		if onLeaf != nil {
			if err := onLeaf(paths, leaf); err != nil {
				return err
			}
		}
		var obj entity.StateAccount
		if err := rlp.Decode(bytes.NewReader(leaf), &obj); err != nil {
			return err
		}
		syncer.AddSubTrie(obj.Root, hexpath, parent, onSlot)
		syncer.AddCodeEntry(entity.BytesToHash(obj.CodeHash), hexpath, parent)
		return nil
	}
