...
type StackTrie struct {
	...
	db       typedb.KeyValueWriter //指向提交数据库的指针，可以为零
}
...
// 如果entrie trie仍然没有散列，Commit将首先对其进行散列，然后将所有节点提交到相关数据库。
//实际上，大多数trie节点可能已经提交。这里的主要目的是提交根节点。需要关联的数据库，否则应禁用整个提交功能。
func (st *StackTrie) Commit() (h entity.Hash, err error) {
	if st.db == nil {
		return entity.Hash{}, ErrCommitDisabled
	}
	...
	//如果节点的RLP不是32字节长，则不会对节点进行散列（并提交），而是包含节点的RLP编码。
	//对于顶级节点，我们需要强制哈希+提交。
	hasher.sha.Reset()
	hasher.sha.Write(st.val)
	hasher.sha.Read(h[:])
