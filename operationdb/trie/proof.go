...
func proofToPath(rootHash entity.Hash, root node, key []byte, proofDb typedb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
	// resolveNode retrieves and resolves trie node from merkle proof stream
	resolveNode := func(hash entity.Hash) (node, error) {
	...
	for {
		...
		case *fullNode:
			...
			continue
		case hashNode:
			child, err = resolveNode(entity.BytesToHash(cld))
			if err != nil {
				return nil, nil, err
			}
		case valueNode:
