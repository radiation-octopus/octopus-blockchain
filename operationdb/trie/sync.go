...
type request struct {
	...
	hash entity.Hash // Hash of the node data content to retrieve
...
type syncMemBatch struct {
	nodes map[entity.Hash][]byte // In-memory membatch of recently completed nodes
	codes map[entity.Hash][]byte // In-memory membatch of recently completed codes
}

// newSyncMemBatch allocates a new memory-buffer for not-yet persisted trie nodes.
func newSyncMemBatch() *syncMemBatch {
	return &syncMemBatch{
		nodes: make(map[entity.Hash][]byte),
		codes: make(map[entity.Hash][]byte),
	}
}

// hasNode reports the trie node with specific hash is already cached.
func (batch *syncMemBatch) hasNode(hash entity.Hash) bool {
	_, ok := batch.nodes[hash]
	return ok
}

// hasCode reports the contract code with specific hash is already cached.
func (batch *syncMemBatch) hasCode(hash entity.Hash) bool {
	_, ok := batch.codes[hash]
	return ok
}

// Sync is the nodeentity state trie synchronisation scheduler, which provides yet
// unknown trie hashes to retrieve, accepts node data associated with said hashes
// and reconstructs the trie step by step until all is done.
type Sync struct {
	database typedb.KeyValueReader    // Persistent database to check for existing entries
	membatch *syncMemBatch            // Memory buffer to avoid frequent database writes
	nodeReqs map[entity.Hash]*request // Pending requests pertaining to a trie node hash
	codeReqs map[entity.Hash]*request // Pending requests pertaining to a code hash
...
func NewSync(root entity.Hash, database typedb.KeyValueReader, callback LeafCallback) *Sync {
	ts := &Sync{
		...
		nodeReqs: make(map[entity.Hash]*request),
		codeReqs: make(map[entity.Hash]*request),
		queue:    prque.New(nil),
		fetches:  make(map[int]int),
	}
	ts.AddSubTrie(root, nil, entity.Hash{}, callback)
	return ts
}
...
func (s *Sync) Missing(max int) (nodes []entity.Hash, paths []SyncPath, codes []entity.Hash) {
	var (
		nodeHashes []entity.Hash
		nodePaths  []SyncPath
		codeHashes []entity.Hash
	)
	...
	for _, child := range children {
		...
		if req.callback != nil {
			if node, ok := (child.node).(valueNode); ok {
				var paths [][]byte
				if len(child.path) == 2*entity.HashLength {
					paths = append(paths, hexToKeybytes(child.path))
				} else if len(child.path) == 4*entity.HashLength {
					paths = append(paths, hexToKeybytes(child.path[:2*entity.HashLength]))
					paths = append(paths, hexToKeybytes(child.path[2*entity.HashLength:]))
				}
